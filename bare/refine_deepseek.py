...
class DeepseekRefine:
    """
    Handles the refinement process of synthetic email chains using Deepseek's API.

    This class reads a JSONL file containing base chains (each with a "chain" field),
    constructs a detailed refinement prompt for each entry, submits them to the Deepseek API,
    cleans and aggregates the responses, and saves the outputs.
    """

    def __init__(self, base_model: str, model_name: str = "deepseek-chat"):
        """
        Initializes the DeepseekRefine processor by setting up directories, loading the API key,
        and initializing the Deepseek API client.

        Args:
            base_model (str): Identifier for the base model used (affects directory structure).
            model_name (str, optional): The name of the Deepseek model to use. Defaults to "deepseek-chat".
        """
        self.logger = CustomLogger(name="DeepseekRefine")
        self.logger.ok("DeepseekRefine initialized")
        ...
        self._directory = os.path.join(repo_root, "syntheticdata", "baserefine", "refine", base_model, "deepseek")
    ...
    def _retrieve_refinement_prompt(self, base_text: str) -> str:
        """
        Constructs the refinement prompt for Deepseek's API given a base email chain text.
    ...
    def _submit_prompt(self, prompt: str) -> str:
        """
        Submits a prompt to the Deepseek API and returns the generated text response.
        Retries up to 5 times in case of temporary unavailability.
        ...
        max_retries = 5
        delay = 2
        system_prompt = "You are a helpful assistant that refines email chains."
    ...
    def _clean_sample(self, sample_str: str) -> dict:
        ...
        Removes Markdown formatting and attempts to parse the JSON.

        Args:
            sample_str (str): The raw string containing JSON data.
    ...
    def serialize_response(self, response, filename: str = None) -> str:
        ...
        Args:
            ...
            filename (str, optional): The file name to use. If not provided, a timestamped filename is generated.

        Returns:
            str: The full file path of the serialized file.
        """
        if not filename:
    ...
    def aggregate_results(self) -> None:
        ...
        After processing each file, the original file is moved to a 'processed' subdirectory.
        """
        new_results = []
    ...
    def run_refinement(self, input_jsonl_file: str, output_filename: str = None, number_samples: int = None):
        """
        Executes the full refinement process for processing a dataset of prompts.
        Reads the base dataset, constructs refinement prompts, submits them to Deepseek,
        cleans and serializes the responses, and aggregates the results.

        Args:
            input_jsonl_file (str): Path to the JSONL file containing the base dataset.
            output_filename (str, optional): Base name for output files.
            ...
            for i, task in enumerate(self._tasks):
                ...
                if prompt:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Error cleaning sample for task #{task['custom_id']}: {e}. Using raw response.")
                        response_clean = {"raw": response_raw}
                    self.serialize_response(response_clean, filename=output_filename)
