...
class OpenAIClient:
    def __init__(self, email_generator):
        ...
        This method performs the following tasks:
          ...
          - Creates a directory for storing batch files if it does not already exist.
          - Initializes an internal task list and index counter.
          - Stores the provided email generator for constructing prompts.

        Parameters:
            email_generator (EmailGenerator): An instance of EmailGenerator used to generate email prompts.
        """
        ...
        self._directory = os.path.join(repo_root, "syntheticdata", "gpt-4-turbo")
    ...
    def _append_prompt_task(self, model: str = "gpt-4-turbo", temperature: float = 1.0):
        """
        Append a prompt task to the internal tasks list.

        Each task represents a request to the OpenAI API and includes a custom ID,
        HTTP method, URL endpoint, and request body. The request body contains the model,
        temperature, and a message constructed using the email generator.

        Parameters:
            model (str): The model to be used for the completion (default is "gpt-4-turbo").
            temperature (float): The sampling temperature for the model (default is 1.0).
        ...
        task = {
            ...
            "body": {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": self._email_generator.construct_prompt()
                    },
                ],
            }
        }
        ...
        self.logger.info("Task generated")

    def append_custom_prompt_task(self, prompt: str, model: str = "gpt-4-turbo", temperature: float = 1.0):
        """
    ...
    def run_batch_inference(self, batch_filename: str, number_samples: int = 10):
        ...
        Parameters:
            ...
            number_samples (int): The number of prompt tasks to generate (default is 10).
