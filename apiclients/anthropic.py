...
class AnthropicClient:
    ...
    def __init__(self, email_generator):
        """
        Initialize an AnthropicClient instance.

        This constructor performs the following:
          - Loads the Anthropic API key from the ENV.txt file.
          - Initializes the Anthropic client with a custom HTTP client (with certificate verification disabled).
          - Sets up the directories for storing batch input, output, and aggregated data.
          - Initializes an internal task list and index counter.
          - Stores the provided email_generator used to construct prompt content.

        Parameters:
            email_generator: An object providing a 'construct_prompt' method to generate prompt text.
        """
        ...
        self._directory = os.path.join(repo_root, "syntheticdata", "claude")
    ...
    def _append_prompt_task(self, model: str = "claude-3-5-sonnet-20241022", temperature: float = 1.0):
        """
        Append a prompt task to the internal task list.

        Constructs a Request object using the email_generator's prompt and specified parameters,
        then increments the internal index.
    ...
    def append_custom_prompt_task(self, prompt: str, model: str = "claude-3-5-sonnet-20241022", temperature: float = 1.0):
        """
