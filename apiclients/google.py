...
class GoogleClient:
    ...
    def __init__(self, email_generator):
        self.logger = CustomLogger(name="GoogleClient")
        self.logger.ok("GoogleClient initialized")
        ...
        self._directory = os.path.join(repo_root, "syntheticdata", "gemini")
    ...
    def run_inference(self, number_samples: int = 10):
        ...
        for idx in range(number_samples):
            ...
            # ------------------------------------------------------------------
            task = self._email_generator.construct_prompt()
